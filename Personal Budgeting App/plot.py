...
class Plotter:
    ...
    def generate_bar_plot(self,size_x=500, size_y=500, by="day", window=None, width=10):
        ...
        fig.set_dpi(dpi)
        rects1 = ax.bar(in_x, in_y, label='Income', color="green", width=width, align="edge")
        rects2 = ax.bar(ex_x, ex_y, label="Expense", color="red", width=-width, align="edge")
        ax.set_ylabel("Amount")
        ax.set_xlabel("Date")
        ax.legend()
        ax.xaxis_date()
        location = join(self.temp_dir, "figure{}.png".format(randrange(int(1e20))))
        plt.savefig(location)
        return(location)
    
    def generate_frame(self):
        return(self._create_frame(map(lambda x: x.to_dict(),self.expense.get_all()+self.income.get_all())))
    ...
    def _sum_by(self, dict_list , by="day"):
        ...
        for tr in dict_list:
            ...
            if key in grouped:
                ...
                grouped[key] = tr["amount"]
        return([(date.strptime(key, format), value,) for (key, value) in grouped.items()])
