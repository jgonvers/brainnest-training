...
class WeatherApp:
    def __init__(self):
        ...
        self.apply_btnCW = TKUtils.create_button(self.window, "Current Weather", self.current_weather)
        self.apply_btnCW.grid(row=3,column=0, sticky=W+E)
        self.apply_btnFC = TKUtils.create_button(self.window, "Forecast", self.forecast)
        ...
        self.start_date_lbl = TKUtils.create_label(self.window, "Input start date")
    ...
    def current_weather(self):
        self.flag = False
        lbl = TKUtils.create_label(self.window, "---- Current Weather ----", bg='lightblue')
    ...
    def forecast(self):
        self.flag = True
        lbl = TKUtils.create_label(self.window, "---- Forecast ----", bg='lightyellow')
        lbl.grid(row=self.x, column=0, columnspan=2)
        self.x += 1
        self.input_vals()
        self.iterate_dic(1)
    ...
    def iterate_dic(self, ind):
        ...
        for y in self.r['data'][ind]:
            for key, value in y.items():
                print(f"{key} = {value}")
                TKUtils.create_label(self.window, f"{key} = {value}", width= 36).grid(row=self.x, column=0, columnspan=2)
                self.x += 1
            if self.flag:
                TKUtils.create_label(self.window, "-------------", bg='lightgrey').grid(row=self.x, column=0, columnspan=2, sticky=W+E)
                self.x += 1       
