            ...
            for key in res:
                match key:
                    case "time"|"sunrise"|"sunset":
                        WeatherUtils.distribute(list(map(datetime.fromisoformat, res[key])), key, converted)
                    case "weathercode":
                        WeatherUtils.distribute(list(map(WeatherUtils.convert_wmo, res[key])), key, converted)
                    case "winddirection_10m_dominant"|"winddirection":
                        WeatherUtils.distribute(list(map(WeatherUtils.convert_wind_direction, res[key])), key, converted)
                    case _:
